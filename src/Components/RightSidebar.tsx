            ...
            <div className='flex flex-col flex-1 justify-between h-full overflow-hidden gap-x-2 items-center'>
                ...
                <div className='h-full rightSidebar overflow-auto mb-2 flex flex-col gap-y-2 rounded-xl'>
                    ...
                    {cartItems.length === 0 ? (
                        ...
                        cartItems.map((item) => (
                            <div key={item.item.id + item.size} className='w-full p-2 group flex bg-zinc-50 rounded-xl'>
                                ...
                                <div className='flex-1 text-zinc-600 pl-2'>
                                    ...
                                    <p>{item.qty}x {item.size}</p> {/* Display the size here */}
                                </div>

                                <div className='flex flex-col justify-between items-end'>
                                    <h1>Rs <span className='font-semibold text-black'>{(item.price * item.qty).toFixed
                                        (2)}</span></h1> {/* Display the price here */}

                                    <div className="w-[7rem] h-[2.5rem] flex justify-between items-center bg-zinc-100 rounded-full">
                                        <span onClick={() => decreaseQuantity(item.item.id, item.size)} className="h-full cursor-pointer aspect-square rounded-full bg-white flex justify-center items-center hover:bg-zinc-300">
                                        ...
                                        <span
                                            ...
                                            onClick={() => addToCart(item.item.id, item.item, item.size, item.price)} // Add item to cart with selected size and price
                                        >
