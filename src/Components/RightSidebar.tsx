...
const RightSidebar = () => {
    const { cart, addToCart, decreaseQuantity } = useCart();  // Get cart items from CartContext
            ...
            <div className='flex flex-col flex-1 justify-between h-full overflow-hidden gap-x-2 items-center'>
                ...
                <div className='h-full rightSidebar overflow-auto mb-2 flex flex-col gap-y-2 rounded-xl'>
                    ...
                    {cartItems.length === 0 ? (
                        ...
                        cartItems.map((item) => (
                            <div key={item.item.id} className='w-full p-2 flex bg-zinc-50 rounded-xl'>
                                <div className='rounded-xl overflow-hidden w-1/4 aspect-square'>
                                ...
                                <div className='flex-1 text-zinc-700 pl-2'>
                                    ...
                                    <p>{item.qty}x</p>
                                    {/* <p>{item.item.specialInstructions || 'No special requirements'}</p> */}
                                </div>

                                <div className='flex flex-col justify-between items-end'>
                                    <h1>Rs {item.item.price * item.qty}</h1>
                                    <div className="w-[7rem] h-[2.5rem] flex justify-between items-center bg-zinc-100 rounded-full">
                                        <span onClick={() => decreaseQuantity(item.item.id)} className="h-full aspect-square rounded-full bg-white flex justify-center items-center hover:bg-zinc-300">
                    ...
                    <div className='bg-zinc-100 p-3 rounded-[24px]'>
                        ...
                        <div className='flex flex-col w-full text-zinc-600 mb-4'>
                            <div className='flex justify-between items-center'>
                                <h1>Sub Total</h1>
                                <h1>Rs {cartItems.reduce((total, item) => total + item.item.price * item.qty, 0)}</h1>
                            </div>
                            ...
                            <div className='flex justify-between items-center pt-2 border-t border-dashed text-black font-semibold border-zinc-400'>
                                <h1>Total Payment</h1>
                                <h1>Rs {(cartItems.reduce((total, item) => total + item.item.price * item.qty, 0) + 10).toFixed(2)}</h1>
                            </div>
                        </div>
                        ...
                        <div className='grid grid-cols-3 gap-x-2 rounded-[20px]'>
                            <div
                                ...
                                className={`w-full py-2 rounded-2xl flex flex-col justify-center items-center gapy-2 border ${paymentMethod == 'esewa' ? ' bg-primary-100/80 border-primary-400' : 'border-transparent bg-white'}`}
                            >
                            ...
                            <div
                                ...
                                className={`w-full py-2 rounded-2xl flex flex-col justify-center items-center gapy-2 border ${paymentMethod == 'cash' ? ' bg-primary-100/80 border-primary-400' : 'border-transparent bg-white'}`}
                            >
                            ...
                            <div
                                ...
                                className={`w-full py-2 rounded-2xl flex flex-col justify-center items-center gapy-2 border ${paymentMethod == 'paylater' ? ' bg-primary-100/80 border-primary-400' : 'border-transparent bg-white'}`}
                            >
